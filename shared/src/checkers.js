...
function wouldPromote(piece, toRow) {
    if (piece.type === 'king')
        return false;
    if (piece.color === 'red' && toRow === 0)
        return true;
    if (piece.color === 'black' && toRow === BOARD_SIZE - 1)
        return true;
    return false;
}
// Find all capture moves from a position (including multi-jumps)
function findCaptureMoves(board, pos, piece, captures = [], visited = new Set()) {
    const moves = [];
    const directions = piece.type === 'king'
        ? [{ row: -1, col: -1 }, { row: -1, col: 1 }, { row: 1, col: -1 }, { row: 1, col: 1 }]
        : getDirections(piece);
    for (const dir of directions) {
        const jumpOver = { row: pos.row + dir.row, col: pos.col + dir.col };
        const landOn = { row: pos.row + dir.row * 2, col: pos.col + dir.col * 2 };
...
function findSimpleMoves(board, pos, piece) {
    const moves = [];
...
export function getValidMovesForPiece(board, pos) {
...
export function isValidMove(board, color, move) {
    const piece = getPiece(board, move.from);
    if (!piece || piece.color !== color)
        return false;
    const validMoves = getAllValidMoves(board, color);
    return validMoves.some(m => m.from.row === move.from.row &&
